...
namespace
{
    ...
    static const array<int, 5 * 6 * 4>& get_window_index_table()
    {
        static const auto build = []()
        {
            array<int, 5 * 6 * 4> table{};
            table.fill(-1);
            const auto& canon_map = get_canon_map();
            for (int x = 0; x < 5; ++x)
            {
                for (int y = 0; y < 6; ++y)
                {
                    ...
                    {
                        const pair<int, int> dirs[4] = {{1, 0}, {0, 1}, {1, 1}, {1, -1}};
                        const int dx = dirs[dir_idx].first;
                        const int dy = dirs[dir_idx].second;
                        vector<pair<int, int>> cells;
                        bool valid = true;
                        for (int step = 0; step < 4; ++step)
                        {
                        ...
                        const string canon = canonical_key_cells(cells);
                        const auto it = canon_map.find(canon);
                        if (it == canon_map.end())
                        {
                            cerr << "Error: canonical key not found for window starting at (" << x << "," << y << ") dir_idx=" << dir_idx << " canon=" << canon << "\n";
                            assert(false && "canonical key lookup failed during table build");
                        }
                        table[(x * 6 + y) * 4 + dir_idx] = it->second;
                    }
                }
            }
            return table;
                                           ...
                                           int min_x, int max_x, int min_y, int max_y)
    {
        ...
        const pair<int, int> dirs[4] = {{1, 0}, {0, 1}, {1, 1}, {1, -1}};
        float score = 0.0f;
        const auto& idx_table = get_window_index_table();
        ...
        for (int x = min_x; x <= max_x; ++x)
        {
            for (int y = min_y; y <= max_y; ++y)
            {
                for (int dir_idx = 0; dir_idx < 4; ++dir_idx)
                {
                    ...
                    const int idx = idx_table[(x * 6 + y) * 4 + dir_idx];
                    if (idx < 0 || idx >= 21)
                    {
                        continue;
                    }
