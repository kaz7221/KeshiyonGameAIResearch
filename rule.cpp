...
constexpr int DEFAULT_TRAIN_AI_DEPTH = 7;      // 学習時の探索深さ
constexpr int DEFAULT_EVAL_AI_DEPTH = 7;       // 評価時の探索深さ
constexpr int ELITE_COUNT = 2;                 // エリート保存数
constexpr int TOURNAMENT_SIZE = 4;             // トーナメント選択のサイズ
constexpr int RANDOM_EVAL_INTERVAL = 10;       // ベンチマークの実施間隔
constexpr int RANDOM_EVAL_GAMES_PER_SIDE = 500;
constexpr float MUTATION_RATE_START = 0.1f;
constexpr float MUTATION_RATE_END = 0.01f;      // 突然変異の確率（最終世代）
constexpr float MUTATION_SCALE_START = 20.0f;  // 突然変異の大きさ（第0世代）
constexpr float MUTATION_SCALE_END = 0.5f;   // 突然変異の大きさ（最終世代）
// =======================
...
int main()
{
    ...
    const int train_ai_depth = DEFAULT_TRAIN_AI_DEPTH;
    const int eval_ai_depth = DEFAULT_EVAL_AI_DEPTH;
    ...
    ga_log << "# TOURNAMENT_SIZE=" << TOURNAMENT_SIZE
           ...
           << " TRAIN_AI_DEPTH=" << train_ai_depth
           << " EVAL_AI_DEPTH=" << eval_ai_depth << '\n';
    ...
    for (int roop = 0; roop < GENERATIONS; roop++)
    {
        ...
        evaluate_population_parallel(population, train_ai_depth);
        ...
        // GA CSV logging: generation,best,mean,worst,weight_std,mutation_scale,w1..wL
        {
            ...
            if (ga_csv)
            {
                if (roop == 0) // write header if starting a run (may duplicate if file reused)
                {
                    ...
                    ga_csv << "generation,best,mean,worst,weight_std,mutation_rate,mutation_scale";
                ...
                // 当該世代で使用する突然変異率と突然変異幅（ともに線形減衰）
                const float t_ratio = (GENERATIONS > 1)
                    ? static_cast<float>(roop) / static_cast<float>(GENERATIONS - 1)
                    : 0.0f;
                const float current_mutation_rate = MUTATION_RATE_START * (1.0f - t_ratio) + MUTATION_RATE_END * t_ratio;
                const float logged_scale = MUTATION_SCALE_START * (1.0f - t_ratio) + MUTATION_SCALE_END * t_ratio;

                // write CSV line
                ga_csv << roop << ',' << best << ',' << mean << ',' << worst << ',' << weight_std << ',' << current_mutation_rate << ',' << logged_scale;
        ...
        if (roop % RANDOM_EVAL_INTERVAL == 0 || roop == GENERATIONS - 1)
        {
            ...
            const auto random_eval_pair = random_eval::evaluate_against_random(
                population.front().second, eval_ai_depth, RANDOM_EVAL_GAMES_PER_SIDE);
            ...
            const auto walk231 = random_eval::evaluate_against_random_walk_231_all(population.front().second, eval_ai_depth, 4);
            ...
            const auto walk232 = random_eval::evaluate_against_random_walk_232_all(population.front().second, eval_ai_depth);
        ...
        // 突然変異率・突然変異幅ともに線形減衰する。
        // 序盤は大きく探索し、集団が収束する後半では細かく探索する。
        // 固定値では、大きすぎると集団が収束せず、小さすぎると収束後に
        // 良個体を壊してしまう（実測: scale=20 で weight_std が 68 のまま横ばい、
        // scale=5 では第10世代で 23 まで落ちた後は平均適応度が低下）。
        const float t_ratio = (GENERATIONS > 1)
            ? static_cast<float>(roop) / static_cast<float>(GENERATIONS - 1)
            : 0.0f;
        const float current_mutation_rate =
            MUTATION_RATE_START * (1.0f - t_ratio) + MUTATION_RATE_END * t_ratio;
        const float current_mutation_scale =
            MUTATION_SCALE_START * (1.0f - t_ratio) + MUTATION_SCALE_END * t_ratio;

        // トーナメント選択: 集団から TOURNAMENT_SIZE 個体をランダムに選び、
        // その中で適応度が最良の個体を親とする。
        // 適応度比例選択では、集団内総当たりのスコア分布が中央に集中するため
        // 選択圧がほとんどかからなかった（best/mean が約1.4倍）。
        uniform_int_distribution<int> parent_dist(0, POPULATION_SIZE - 1);
        auto select_parent_tournament = [&]() -> int
        {
            int best_idx = parent_dist(engine);
            for (int k = 1; k < TOURNAMENT_SIZE; ++k)
            {
                const int cand = parent_dist(engine);
                if (population[cand].first > population[best_idx].first)
                {
                    best_idx = cand;
                }
            }
        ...
        {
            const int p1 = select_parent_tournament();
            const int p2 = select_parent_tournament();
            individual child(population[p1].second, population[p2].second);
            child.mutate(current_mutation_rate, current_mutation_scale);
